...
### YOLOv8n Inference · Kria KR260 · 300 MHz · v2.1
...
- [ ] Implement FENCE barrier: stall fetch until unit_done bitmask clears
- [ ] Implement global stall: when current target FIFO is full, hold fetch FSM; do not advance instr_ptr
- [ ] Implement AXI-Lite write-only slave (AW+W+B channels; no read channels): decode instr_base, instr_count, kick
...
## 14. Changes from v2.0

### 14.1 Sequencer Stall Policy — Global Stall on Target FIFO Full

**Decision:** When the sequencer's current instruction targets a unit FIFO that is full, the sequencer halts DDR4 fetch entirely and retries every cycle until the push succeeds. It does not skip ahead or reorder instructions.

**Stall condition (RTL):**
```systemverilog
assign dispatch_stall = (current_unit != UNIT_SEQ) & fifo_full[current_unit_idx];
```
FENCE and CONFIG (`unit_id == UNIT_SEQ`) are never pushed to a FIFO and are exempt from `fifo_full` stalls.

**Why not per-unit lookahead / out-of-order dispatch:**

An out-of-order dispatcher would need to buffer fetched-but-not-yet-dispatched instructions in a secondary FIFO, maintain a per-unit availability scoreboard, and verify that skipping instruction N does not violate the ordering assumptions the ARM driver used when encoding the dep token bits. The ARM driver was written assuming strict in-order issue — no instruction in the stream was encoded with dep tokens conservative enough to tolerate reordering. Building a reorder buffer to safely skip ahead would add ~100–200 LUTs of scoreboard logic, a prefetch FIFO (128-bit wide, N-deep), and a hazard detection unit, for a correctness risk that isn't worth the marginal throughput gain. `[Source: apache/tvm-vta — VTA fetch FSM stalls on cmd_queue_full, never reorders; github.com/apache/tvm-vta]`

**Why global stall is safe and sufficient:**

The per-unit FIFOs are sized to absorb the sequencer-to-unit speed mismatch (DMA=16, SA=32, PSB=32, REQ=8, VPU=16). At 300 MHz, with FENCE barriers resetting FIFO fill at every layer boundary, a FIFO-full stall is uncommon in the YOLOv8n instruction stream. When it does occur, it stalls only for the cycles needed to drain one slot — the unit FSMs are already pulling concurrently. The dep token system continues to provide cross-unit parallelism regardless of whether the sequencer is momentarily stalled. `[Source: ISA_Architecture_QA.md §11 — "The per-unit FIFOs absorb this mismatch"; §5 — pipeline concurrency via dep tokens, not sequencer lookahead]`

**RTL resource cost:** One 128-bit shift register (4 × 32-bit FFs) to assemble the in-flight instruction. No prefetch FIFO. No scoreboard. `[Source: no canonical citation — engineering judgment]`

### 14.2 AXI-Lite Slave — Write-Only, Read Channels Removed

**Decision:** The AXI-Lite slave on the sequencer implements only the AW + W + B channels (write-only). The AR and R channels are omitted entirely. `s_axil_wstrb` is also omitted.

**Reasoning:** The three CSR registers (`instr_base`, `instr_count`, `kick`) are write-only from the ARM's perspective. The ARM driver never reads them back — it writes them once per layer dispatch and waits on the completion IRQ. Implementing read channels adds 7 signals to the port list and requires read-address handshake logic and a read-data mux for zero benefit. `kick` is implemented as a write-triggered 1-cycle internal strobe rather than a level signal, eliminating the need for the ARM to explicitly clear it with a second transaction (which would be required with AXI GPIO — ruled out for this reason). `[Source: ISA_Architecture_QA.md §22 — "ARM writes three CSR registers once per layer dispatch"; ZipCPU easyaxil — write-only slave pattern]`

---

*Document version: 2.1 — sequencer stall policy formalized (global stall); AXI-Lite slave reduced to write-only (AW+W+B, no AR/R).*
